...
import { getUserPasswordHash, getUserRecoveryCode, resetUserRecoveryCode, updateUserPassword } from "$lib/server/user";
import {
	createSession,
...
async function updatePasswordAction(event: RequestEvent) {
	...
	if (!passwordUpdateBucket.check(event.locals.session.sessionId, 1)) {
		return fail(429, {
			password: {
	...
	if (!passwordUpdateBucket.consume(event.locals.session.sessionId, 1)) {
		return fail(429, {
			password: {
				message: "Too many requests"
			}
